...
// Раньше тут стояла прямая ссылка на функцию: addEventListener('click', showWiki).
// showWiki() определена в wikiView.js, который в index.html подключается
// ПОСЛЕ mapView.js — на момент выполнения этой строки (верхний уровень
// скрипта, выполняется сразу при загрузке страницы, а не по клику)
// идентификатор showWiki ещё не существует ни в одном подключённом
// скрипте → ReferenceError прямо здесь. Он прерывает выполнение всего
// оставшегося mapView.js (в том числе цикл ниже, который вешает клики на
// пункты меню "Вики ▾" и на четыре его раздела) — поэтому не открывался
// НИ ОДИН раздел вики, ни через дропдаун, ни через клик на #brand: их
// обработчики клика просто никогда не регистрировались. Обёртка в
// стрелочную функцию откладывает разрешение имени showWiki до момента
// реального клика, когда все скрипты уже точно загружены.
